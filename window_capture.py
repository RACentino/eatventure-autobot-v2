...
def configure_overlay_canvas(
    canvas: Any, background: str = TRANSPARENT_OVERLAY_COLOR
) -> None:
...
def start_overlay_process(name: str, target: Any, *args: Any) -> tuple[Any, Any]:
    ...
    process = context.Process(
        target=target, name=name, args=(*args, stop_event), daemon=True
    )
    process.start()
...
def stop_overlay_process(
    process: Any | None,
    stop_event: Any | None,
    timeout: float = OVERLAY_PROCESS_JOIN_TIMEOUT,
) -> None:
...
def _set_platform_transparency(root: Any) -> bool:
    ...
    if system_name == "Windows":
        return _set_window_attribute(
            root, "-transparentcolor", TRANSPARENT_OVERLAY_COLOR
        )
...
def _enable_windows_click_through(window_id: int) -> bool:
    ...
    new_style = (
        style
        | WINDOWS_WS_EX_LAYERED
        | WINDOWS_WS_EX_TRANSPARENT
        | WINDOWS_WS_EX_TOOLWINDOW
    )
...
def _set_x11_shape_regions(
    window_id: int, shape_kind: int, regions: list[OverlayRegion]
) -> bool:
    ...
    if x11_path is None or xfixes_path is None:
        return False
    return _apply_x11_shape_regions(
        window_id, shape_kind, regions, display_name, x11_path, xfixes_path
    )
...
def _create_x11_region(xfixes: Any, display: Any, regions: list[OverlayRegion]) -> int:
    ...
    rectangles = (_XRectangle * len(regions))(
        *[_x_rectangle(region) for region in regions]
    )
...
def _find_overlay_window_rect(window_title: str) -> WindowRect | None:
    ...
    try:
        ...
        logger.debug(
            "Overlay could not search for target window '%s': %s", window_title, exc
        )
        return None
...
def _run_forbidden_area_overlay_process(
    window_title: str, forbidden_zones: tuple[ForbiddenZone, ...], stop_event: Any
) -> None:
    try:
        ...
        _schedule_forbidden_overlay_draw(
            root, canvas, window_title, forbidden_zones, stop_event
        )
        root.mainloop()
    except Exception as exc:
...
def _schedule_forbidden_overlay_draw(
    root: Any,
    canvas: Any,
    window_title: str,
    ...
    root.after(
        FORBIDDEN_AREA_OVERLAY_REFRESH_MS,
        _schedule_forbidden_overlay_draw,
        root,
        canvas,
        window_title,
...
class ForbiddenAreaOverlay:
    def __init__(
        self, window_capture: WindowCapture, forbidden_zones: list[ForbiddenZone]
    ) -> None:
    ...
    def start(self) -> None:
        ...
        try:
            ...
            self.process, self.stop_event = start_overlay_process(
                "forbidden_area_overlay",
                _run_forbidden_area_overlay_process,
    ...
    for x_min, x_max, y_min, y_max in zones:
        regions.append(
            (
                int(x_min),
                int(y_min),
                max(1, int(x_max) - int(x_min)),
                max(1, int(y_max) - int(y_min)),
            )
        )
    return regions
