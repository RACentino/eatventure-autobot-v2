"""Same-state stall watchdog. Verified behavior in both source repos: force a transition back to
a fallback state (FIND_RED_ICONS) after state_stall_timeout_seconds in the same state, forever,
with no escalation. This module keeps that self-healing behavior but adds a bounded-retry-then-
